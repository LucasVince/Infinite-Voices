...
export const registerUser: RequestHandler = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        const token = jwt.sign({ id: user._id, username: user.username }, process.env.REGISTER_USER_SECRET_KEY as string, { expiresIn: '1h' });
...
export const loginUser: RequestHandler = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        const token = jwt.sign({ id: user._id, username: user.username }, process.env.LOGIN_USER_SECRET_KEY as string, { expiresIn: '1h' });
        res.status(200).json({ token, user });
    } catch (err) {
        console.error(err);
